...
from .utils import clean_ir, find_all_values, find_all_identifiers, tokenize_ir
...
class Instruction(IRUnit):
    def __init__(self, inst_str: str, bb_names: list[str], is_terminal: bool = False):
        super().__init__()
        self.string = " ".join(inst_str.split())
...
class BasicBlock(IRUnit):
    def __init__(self, bb_str: str, bb_names: list[str]):
        super().__init__()
        string = bb_str.strip(" \n\r\t{}")
        match = re.match(
        ...
        for char in body:
            ...
            if char in "([{":
                ...
                if num_open_brackets == 0:
                    inst_strings.append(matched_string.strip())
        ...
        self.instructions = []
        for i, inst_string in enumerate(inst_strings):
            inst = Instruction(
                inst_string,
                bb_names=bb_names,
                is_terminal=True if i == len(inst_strings) - 1 else False,
            )
            self.instructions.append(inst)

        # self.has_return = False
        # if self.instructions[-1].is_return:
        #     self.has_return = True

    # def extract_insts(self):
    #     inst_strings = []
    #     matched_string = ""
    #     num_open_brackets = 0
...
class Function(IRUnit):
    def __init__(self, func_str: str):
        super().__init__()
        string = func_str.strip()
        match = re.match(
            r"^define\s+.*?(@[-a-zA-Z$._0-9]+)\s*\((.*?)\).*?\{(.+)}",
            string,
            flags=re.S | re.M,
        )
        self.name = match.group(1).strip()
        self.args = find_all_identifiers(match.group(2).strip())
        self.args_string = match.group(2).strip()
        body = match.group(3).strip()
        self.basic_blocks = []
        self.bb_names = re.findall(r"^([a-zA-Z0-9_.]+):", string, flags=re.M)
        bb_matches = re.findall(
            r"^[a-zA-Z0-9_.]+:.*?(?=^[a-zA-Z0-9_.]+:|\Z)", body, flags=re.S | re.M
        )
        for bb_match in bb_matches:
            bb = BasicBlock(bb_match, bb_names=self.bb_names)
            self.basic_blocks.append(bb)

        # number of basic blocks
        # self.num_bbs = len(self.basic_blocks)
...
class IRFile(IRUnit):
    def __init__(self, ir_str: str, cfg_vocab, dfg_vocab, build_graph=True):
        ...
        func_matches = re.findall(
            r"^define\s+.*?@\w+\s*\(.*?\)\s*.+?(?=^define|\Z)",
            ir_str,
            flags=re.S | re.M,
        )
        for func_match in func_matches:
            function = Function(func_match)
...
def parse_ir_file(
    ...
    try:
        llvm_ir = clean_ir(content)
        return IRFile(llvm_ir, cfg_vocab, dfg_vocab, build_graph=build_graph)
    except Exception:
        return None
